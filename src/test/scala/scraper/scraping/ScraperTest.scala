...
//  }
  ...
  it should "scrape first chapter" in {
    test("1373411-Tiny-Life-Guard", "1") { chapter =>
      ...
      assert(
        chapter.choices(0).name.startsWith("Karen call Juli down"),
        chapter.choices
      )
      assert(
        chapter
          .choices(1)
          .name
          .startsWith("Get outta my office and find Juli!!!"),
        chapter.choices
      )
      assert(
        chapter.choices(2).name.startsWith("I will give you a tour"),
        chapter.choices
      )
      assert(
        chapter.choices(3).name.startsWith("Kim train him"),
        chapter.choices
      )
      assert(
        chapter.choices(4).name.startsWith("Go take a self tour"),
        chapter.choices
      )
    }
  }
    ...
    test("1373411-Tiny-Life-Guard", "112222111122") { chapter =>
      ...
      assert(
        chapter
          .choices(0)
          .name
          .startsWith("Kim was telling the truth, Jim is under Juli."),
        chapter.choices
      )
      assert(
        chapter
          .choices(1)
          .name
          .startsWith("Kim is lying, Jim is still flatten to her butt."),
        chapter.choices
      )
    }
  }
  ...
  it should "scrape another normal chapter" in {
    test("2188228-Shrink-as-you-want-REWARDS", "1343221") { chapter =>
      ...
      assert(
        chapter.choices(0).name.startsWith("In between the"),
        chapter.choices
      )
      assert(
        chapter.choices(1).name.startsWith("Inside her Bag."),
        chapter.choices
      )
    ...
    test("1373411-Tiny-Life-Guard", "1151111112221211") { chapter: Chapter =>
      ...
      assert(
        chapter
          .choices(0)
          .name
          .contains("She sees you, and manages to peel you off."),
        chapter.choices
      )
      assert(
        chapter
          .choices(1)
          .name
          .contains("You have tanned with Elsa! She can't see you!"),
        chapter.choices
      )
    }
  }
  ...
  it should "scrape a messed up chapter" in {
    ...
    test("1384303-Shrunken-at-Home", "1353112211212122112112121") { chapter =>
